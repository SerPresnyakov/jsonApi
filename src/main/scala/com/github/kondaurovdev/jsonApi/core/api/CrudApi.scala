...
import com.github.kondaurovdev.jsonApi.core.api.request.{UpdateRequest, CreateRequest, RetrieveRequest}
...
case class CrudApi(table: iTableCrud) {
  ...
  def doRequest(request: crudRequest): Either[JsValue, JsValue] = request match {
    ...
    case get: RetrieveRequest =>
      table.getAll(get.filters.getOrElse(List()): _*)(Some(get.pager))
